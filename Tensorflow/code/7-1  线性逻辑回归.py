...
def generate(sample_size, mean, cov, diff, regression):
    ...
    X0 = np.random.multivariate_normal(mean, cov, samples_per_class)  # 多元正态随机分布
    ...
    # one-hot  0 into the vector "1 0  将0 转换为向量 0 1
    if regression == False:
...
# Set model weights 权中的值和X的个数相同,维度也相同
...
optimizer = tf.train.AdamOptimizer(0.04)  # 尽量用Adam这个--收敛快，会动态调节梯度
...
with tf.Session() as sess:
    ...
    for epoch in range(maxEpochs):
        ...
        for i in range(np.int32(len(Y) / minibatchSize)):
            x1 = X[i * minibatchSize:(i + 1) * minibatchSize, :]  # 分批次训练
