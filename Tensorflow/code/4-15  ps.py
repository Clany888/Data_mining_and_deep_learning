...
cluster_spec = tf.train.ClusterSpec({'ps': ps_hosts, 'worker': worker_hosts})  # 创建三个终端 1个ps,2个worker
# 创建server
...
sv = tf.train.Supervisor(is_chief=(task_index == 0), logdir="log/super/", init_op=init, summary_op=None, saver=saver,
                         global_step=global_step, save_model_secs=5)
...
with sv.managed_session(server.target) as sess:
    ...
    for epoch in range(global_step.eval(session=sess), training_epochs * len(train_X)):
        # zip 函数用于将可迭代的对象作为参数,将对象中对于的元素打包成一个个元组,返回由这些元组组成的列表
        for (x, y) in zip(train_X, train_Y):
            ...
            if epoch % display_step == 0:
                ...
                if not (loss == "NA"):
                    ...
                    plotdata["loss"].append(loss)  # sv.saver.save(sess,"log/mnist_with_summaries/",global_step=epoch)
