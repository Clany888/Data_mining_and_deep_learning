...
def onehot(y, start, end):
    ...
    ohe.fit(b)
    c = ohe.transform(y).toarray()  # transform()将降维应用于y
    return c
...
def generate(sample_size, num_classes, diff, regression=False):
    ...
    for ci, d in enumerate(diff):
        ...
        Y0 = np.concatenate((Y0, Y1))  # print(X0, Y0)
    
    if regression == False:  # one-hot  0 into the vector "1 0
        Y0 = np.reshape(Y0, [-1, 1])
        # print(Y0.astype(np.int32))
        Y0 = onehot(Y0.astype(np.int32), 0, num_classes)  # print(Y0)
    X, Y = shuffle(X0, Y0)
    # print(X, Y)
    return X, Y
...
with tf.Session() as sess:
    ...
    xx, yy = np.meshgrid(xs1, xs2)  # create the grid  从坐标向量返回坐标矩阵。
    ...
    cmap = ListedColormap([colorConverter.to_rgba('r', alpha=0.30), colorConverter.to_rgba('b', alpha=0.30),
                           colorConverter.to_rgba('y', alpha=0.30)])
    # Plot the classification plane with decision boundary and input samples
    plt.contourf(xx, yy, classification_plane, cmap=cmap)  # 画三维等高图的
    plt.show()
